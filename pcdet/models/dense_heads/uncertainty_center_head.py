...
from ..dense_heads.center_head import CenterHead
...
class UncertaintyCenterHead(CenterHead):
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            pred_uncertainties = torch.cat([pred_dict[f"{head_name}_uncertainty"] for head_name in self.separate_head_cfg.HEAD_ORDER], dim=1)

            reg_loss = self.reg_loss_func(
                pred_boxes, target_dicts['masks'][idx], target_dicts['inds'][idx], target_boxes, pred_uncertainties
            )
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            batch_center_uncertainty = pred_dict['center_uncertainty']
            batch_center_z_uncertainty = pred_dict['center_z_uncertainty']
            batch_dim_uncertainty = pred_dict['dim_uncertainty']
            batch_rot_uncertainty = pred_dict['rot_uncertainty']
            
            collected_uncertainties = [batch_center_uncertainty, batch_center_z_uncertainty,
                                       batch_dim_uncertainty, batch_rot_uncertainty[:, 0].unsqueeze(dim=1)]
            if batch_vel is not None:
                batch_vel_uncertainty = pred_dict['vel_uncertainty'] if 'vel_uncertainty' in pred_dict \
                    else torch.zeros_like(batch_vel)
                collected_uncertainties.append(batch_vel_uncertainty)
            ...
            elif self.model_cfg.LOSS_CONFIG.UNCERTAINTY_LOSS_TYPE == 'laplace':
                batch_uncertainties = 2 * batch_uncertainties ** 2
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                ...
                final_dict['pred_uncertainties'] = final_dict['pred_uncertainties'][selected]

                ret_dict[k]['pred_boxes'].append(final_dict['pred_boxes'])
                ret_dict[k]['pred_scores'].append(final_dict['pred_scores'])
                ret_dict[k]['pred_labels'].append(final_dict['pred_labels'])
                ret_dict[k]['pred_uncertainties'].append(final_dict['pred_uncertainties'])

        for k in range(batch_size):
            ret_dict[k]['pred_boxes'] = torch.cat(ret_dict[k]['pred_boxes'], dim=0)
            ret_dict[k]['pred_scores'] = torch.cat(ret_dict[k]['pred_scores'], dim=0)
            ret_dict[k]['pred_labels'] = torch.cat(ret_dict[k]['pred_labels'], dim=0) + 1
            ret_dict[k]['pred_uncertainties'] = torch.cat(ret_dict[k]['pred_uncertainties'], dim=0)

        return ret_dict

    def _activate_fill_uncertainty(self, pred_dict):
        for head_name in self.separate_head_cfg.HEAD_ORDER:
            ...
            pred_dict[f"{head_name}_uncertainty"] = pred_uncertainty
        return pred_dict
    ...
    def forward(self, data_dict):
        ...
        for head in self.heads_list:
            predictions = head(x)
            predictions = self._activate_fill_uncertainty(predictions)
            pred_dicts.append(predictions)
        ...
        if not self.training or self.predict_boxes_when_training:
            ...
            if self.predict_boxes_when_training:
                rois, roi_scores, roi_labels, roi_uncertainties = self.reorder_rois_for_refining(data_dict['batch_size'], pred_dicts)
                data_dict['rois'] = rois
                data_dict['roi_scores'] = roi_scores
                data_dict['roi_labels'] = roi_labels
                data_dict['has_class_labels'] = True
                data_dict['roi_uncertainties'] = roi_uncertainties
            else:
    ...
    def reorder_rois_for_refining(batch_size, pred_dicts):
        ...
        roi_uncertainties = pred_boxes.new_zeros((batch_size, num_max_rois, pred_boxes.shape[-1]))
        ...
        for bs_idx in range(batch_size):
            ...
            roi_uncertainties[bs_idx, :num_boxes] = pred_dicts[bs_idx]['pred_uncertainties']
        return rois, roi_scores, roi_labels, roi_uncertainties
